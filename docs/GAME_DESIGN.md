...
- **Stealth / detection.** *Still open.* The unit rosters (Section 8.7) already assume this system exists — Cohort's Chronicler "reveals stealth," Conclave's Phantom has "illusions/stealth" — but no stealth or detection mechanic is defined anywhere in this document or in `DECISIONS.md`. This is a dependency hiding inside the roster table rather than a standalone idea. It blocks the Chronicler, which in turn blocks D-020's Command Overload mitigation. **Note:** the designer believes this was decided in conversation; a full search of `docs/` on 2026-08-06 found only restatements of the question, never an answer. It needs to be written down once.
